...
Over the course of a day, the economy ebbs and flows. Represent this as a four-section `clock` that defaults to a regular `fortune roll`. Players can burn `money` to represent extra expenditures, which will add +1d6 to the `fortune roll`.
...
The market clock automatically ticks forward once per scene.
